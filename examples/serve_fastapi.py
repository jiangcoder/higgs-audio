        ...
        **kwargs,
        ...
        for idx, chunk_text in enumerate(chunked_text):
            ...
            for ele in outputs[1]:
                audio_out_ids = ele
                logger.info(f"audio output shape: {audio_out_ids.shape}, audio_out_ids {audio_out_ids}")
        ...
        logger.info(f"Final audio generation_chunk_buffer_size {generation_chunk_buffer_size}, output shape: {concat_audio_out_ids.shape}, concat_audio_out_ids {concat_audio_out_ids}")
...
async def generate_audio(request: GenerationRequest):
    ...
    try:
        ...
        logger.info(f"Audio shape after resampling: {audio_np.shape}, sample rate: {request.sample_rate}")
        ...
        elapsed = time.time() - start_time
        logger.error(f"Error during audio generation after {elapsed:.2f}s: {e}")
    ...
    with torch.inference_mode():
        ...
        async for delta in async_streamer:
            if delta.audio_tokens is not None:
                logger.info(f"Received audio tokens, shape: {delta.audio_tokens.shape}")
        ...
        if len(audio_out_ids_l) > 0:
            logger.info(f"All audio tokens collected, total chunks: {len(audio_out_ids_l)}")
            concat_audio_out_ids = torch.concat(audio_out_ids_l, dim=1)
            logger.info(f"Final concatenated audio output shape: {concat_audio_out_ids.shape}")
            ...
            if model_client._config.use_delay_pattern:
                ...
                logger.info(f"After delay pattern revert, audio output shape: {concat_audio_out_ids.shape}")

            # 处理音频token的裁剪
            concat_audio_out_ids = concat_audio_out_ids.clip(0, model_client._audio_tokenizer.codebook_size - 1)[:, 1:-1]
            logger.info(f"After clipping, audio output shape: {concat_audio_out_ids.shape}")
            ...
            actual_sample_rate = model_client._audio_tokenizer.sampling_rate
            logger.info(f"Audio shape after decoding: {wv.shape}, sample rate: {actual_sample_rate}, wv datatype {wv.dtype}")
            
            # 重采样到目标采样率
            if request.sample_rate != actual_sample_rate:
                wv = librosa.resample(wv, orig_sr=actual_sample_rate, target_sr=request.sample_rate)
                logger.info(f"Audio resampled to {request.sample_rate}Hz, shape: {wv.shape}")
